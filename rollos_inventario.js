...
window.initRollosInventario = function () {
  ...
  // Fallback to comercio if calculos is missing
  let calc = window.ROLLOS_RAW.calculos || window.ROLLOS_RAW.comercio || [];
  ...
  RI_DATA = calc.map(r => {
    ...
    // Normalizar nombres de columnas (calculos usa prefix cal_ o directo)
    ...
    return {
      ...
      nombre_sitio:    r.nombre_sitio || meta.nombre_sitio,
      departamento:    r.departamento || meta.departamento,
      ciudad:          r.ciudad       || meta.ciudad,
      proyecto:        r.proyecto     || meta.proyecto,
...
function _chartCoberturaBuckets() {
  ...
  riCharts.cobBuckets = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [{ label: 'Corresponsales', data: Object.values(buckets),
        backgroundColor: ['#FF5C5C','#FFC04D','#DFFF61','#B0F2AE','#99D1FC'],
        borderRadius: 6, borderWidth: 0 }]
    },
    options: { ...RICHART_OPTS, responsive: true, maintainAspectRatio: false,
...
function _chartConsumoVsProyectado() {
  ...
  riCharts.consumoProy = new Chart(ctx, {
    ...
    options: { ...RICHART_OPTS, responsive: true, maintainAspectRatio: false,
      scales: { x: { grid: { color: 'rgba(255,255,255,.04)' }, ticks: { color: '#94a3b8', font: { size: 9 }, maxRotation: 45 } },
                y: { grid: { color: 'rgba(255,255,255,.04)' }, ticks: { color: '#94a3b8', font: { size: 11 } } } },
...
function _chartRotacion() {
  ...
  riCharts.rotacion = new Chart(ctx, {
    ...
    options: { ...RICHART_OPTS, responsive: true, maintainAspectRatio: false, cutout: '62%',
...
function _chartTopSaldos() {
  ...
  riCharts.topSaldos = new Chart(ctx, {
    ...
    options: { ...RICHART_OPTS, responsive: true, maintainAspectRatio: false, indexAxis: 'y',
...
function _chartSLADistrib() {
  ...
  riCharts.sla = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [{ data: [slaOk, slaNo],
        backgroundColor: ['rgba(176,242,174,.8)','rgba(255,92,92,.7)'],
        borderWidth: 0, hoverOffset: 8 }]
    },
    options: { ...RICHART_OPTS, responsive: true, maintainAspectRatio: false, cutout: '65%',
      plugins: { ...RICHART_OPTS.plugins,
...
function _chartCoberturaHistogram() {
  ...
  riCharts.cobHist = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [{ label: 'Corresponsales', data: Object.values(bins),
    ...
    options: { ...RICHART_OPTS, responsive: true, maintainAspectRatio: false,
      scales: { x: { grid: { color: 'rgba(255,255,255,.04)' }, ticks: { color: '#94a3b8', font: { size: 9 } },
      ...
      plugins: { ...RICHART_OPTS.plugins, legend: { display: false },
        annotation: { annotations: {
          sla: { type: 'line', xMin: 6, xMax: 6, borderColor: '#B0F2AE', borderWidth: 2, borderDash: [4,4],
...
function _renderRITableData(data) {
  ...
  wrap.innerHTML = `
    <table style="width:100%;border-collapse:collapse;font-family:'Outfit',sans-serif;font-size:12px;">
      <thead>
        <tr style="background:rgba(0,0,0,.3);border-bottom:1px solid rgba(176,242,174,.15);">
...
window.riGoPage = function(p) { riPage = p; _renderRITable(); };
